...
const XAddSasaran = ({
  ...
  const handleDeleteSasaran = (e: any, id: string) => {
    e.preventDefault();
    const newArray = dataSasaran.filter((item: any) => item.id_sasaran !== id);
  ...
  return (
    <CommonModal isOpen={openAddSasaran} onClose={setOpenAddSasaran}>
      <div className="flex flex-col w-full py-3">
        <div className="relative">
          <div className="flex flex-col justify-between items-center">
            <div className="w-full">
              <div className="text-center font-medium md:text-title-xsm text-title-xsm2 mb-6 bg-meta-6 py-2">
                Masukkan Sasaran
              </div>
              ...
              <div>
                <ul className="mt-[5em] ml-4">
                  {dataSasaran?.length > 0 &&
                    dataSasaran?.map((el: any, i: number) => (
                      <li className="font flex flex-col gap-2" key={i}>
                        <div className="flex justify-between">
                          <div className="flex gap-2">
                            <div
                              className={`${
                                dataSasaran.length > 1 ? "block" : "hidden"
                              }`}
                            >
                              {i + 1} .
                            </div>
